...
function OrderItem({item}) {
   ...
   return (
      <a className="card-item width-100 pad-1-0 my-orders-item surface-hover cursor-pointer no-decoration" href={`/orders/${item.id}`}>
         ...
         <div className="row-1 align-start pad-0-1">
            ...
            <div className="row-1 flex-1">
               <div className="row-1 flex-1">
                  {
                     item.image_sample.length &&
                        <ul className="row gap-025 align-start order-item-img-sample " >
                           {
               ...
               <div className="col-05 align-start">
                  ...
                  <label >{utils.getDateToStrShow(item.delivery_forecast, false)}</label>         
               </div>            
...
export default function MyOrders(props) {
   ...
   return <AppMainContainer>
      <div className="col-1 my-orders-parent">
            ...
            <div className="parent-pagetrack-my-orders">
               {(searchData && searchData.results.length > 0) && 
                  <PageTrack 
                     ...
                     rowOffset={ordersOffset} 
                     onSelectOffset={(val) => setOrdersOffset(val)} 
                     rowTotal={searchData.metadata.total} /> } 
            </div>            
         </header>
         {
            loadState.state === LS_LOADING ?
               (
                  <div className="my-orders-parent-loading">
                     <SurfaceLoading  />
                  </div>
               ) :
               (
         ...
         <footer className="row-05   width-100 border-top-grey just-end pad-1-0">
            {(searchData && searchData.results.length > 0)  && 
               <PageTrack 
                  pageSize={OrdersPageSize} 
                  rowOffset={ordersOffset} 
